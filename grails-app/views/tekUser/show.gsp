	...
	<body>
		...
		<div id="show-tekUser" class="content scaffold-show" role="main">
			...
			<ol class="property-list tekUser">
				...
				<g:if test="${tekUserInstance?.tekEvent}">
				<li class="fieldcontain">
					<span id="tekEvent-label" class="property-label"><g:message code="tekUser.tekEvent.label" default="Tek Event" /></span>
					
						<span class="property-value" aria-labelledby="tekEvent-label"><g:link controller="tekEvent" action="show" id="${tekUserInstance?.tekEvent?.id}">${tekUserInstance?.tekEvent?.encodeAsHTML()}</g:link></span>
