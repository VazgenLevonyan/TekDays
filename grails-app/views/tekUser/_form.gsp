...
<div class="fieldcontain ${hasErrors(bean: tekUserInstance, field: 'tekEvent', 'error')} ">
	<label for="tekEvent">
		<g:message code="tekUser.tekEvent.label" default="Tek Event" />
		
	</label>
	<g:select id="tekEvent" name="tekEvent.id" from="${event.TekEvent.list()}" optionKey="id" value="${tekUserInstance?.tekEvent?.id}" class="many-to-one" noSelection="['null': '']"/>
