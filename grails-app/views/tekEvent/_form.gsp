...
<div class="fieldcontain ${hasErrors(bean: tekEventInstance, field: 'organizer', 'error')} required">
	...
	<g:select id="organizer" name="organizer.id" from="${user.TekUser.list()}" optionKey="id" required="" value="${tekEventInstance?.organizer?.id}" class="many-to-one"/>
...
<li class="add">
<g:link controller="tekSponsorship" action="create" params="['tekEvent.id': tekEventInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'tekSponsorship.label', default: 'TekSponsorship')])}</g:link>
</li>
</ul>
...
<g:each in="${tekEventInstance?.tasks?}" var="t">
    <li><g:link controller="tekTask" action="show" id="${t.id}">${t?.encodeAsHTML()}</g:link></li>
</g:each>
<li class="add">
<g:link controller="tekTask" action="create" params="['tekEvent.id': tekEventInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'tekTask.label', default: 'TekTask')])}</g:link>
</li>
</ul>
...
<li class="add">
	<g:link controller="tekMessage" action="create" params="['event': tekEventInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'tekMessage.label', default: 'TekMessage')])}</g:link>
</li>
</ul>
...
<li class="add">
<g:link controller="tekUser" action="create" params="['tekEvent.id': tekEventInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'tekUser.label', default: 'TekUser')])}</g:link>
</li>
</ul>
